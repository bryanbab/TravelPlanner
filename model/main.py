...
class RouteConfig:
    def __init__(self):
        self.buffer_km = 25  # Search corridor width
        ...
        self.theme = "healthcare"  # Default theme
...
# ==== POI HANDLING ====
api = overpass.API(endpoint="http://localhost:12347/api/interpreter")

def query_pois_for_segment(segment, theme, buffer_km):
    buffer_deg = buffer_km * 1000 / 111320
    buffered = segment.buffer(buffer_deg)
    bbox = (buffered.bounds[1], buffered.bounds[0],
    ...
    try:
        response = requests.post(
            "http://localhost:12347/api/interpreter",
            data={"data": query},
            timeout=30
        )
        response_json = response.json()
        return [(float(e['lon']), float(e['lat'])) for e in response_json.get("elements", [])]
    except:
        return []
...
if __name__ == "__main__":
    ...
    route_geo = generate_random_route("Malden", "Boston", config)
    if route_geo:
        current_score = calculate_score(route_geo, config, user_prefs)
        print("1️⃣  initial score:", round(current_score, 4))
        print("------------------------------")
        
    # Simulated annealing loop sketch
    for iteration in range(5):
        new_config = neighbor_function(config)
        new_route = generate_random_route("Malden", "Boston", new_config)
        print('2️⃣  generated new route')

        if new_route:
            new_score = calculate_score(new_route, new_config, user_prefs)
            print(f"3️⃣  New score: {round(new_score, 4)}")
            print("---------------------------------")
